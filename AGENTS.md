...
src/            viki CLI source (C)
  ...
  viki_index.c/.h `viki index <dir>`: walk+chunk+hash+embed checkout files, plus subprocess extraction of EIGHT virtual namespaces -- `wiki:Name`, `ticket:UUID`, `forum:UUID`, `ckin:UUID` (check-in comments), `note:ID` (tech notes), `tchg:UUID` (ticket change history), `attach:UUID`, `uv:NAME`. Every non-file class is read with ONE `fossil sql` call using a counted framing (`framed_next()`), not one subprocess per artifact: each `fossil` invocation costs a process spawn plus repo open, so the per-artifact idiom is O(artifacts) subprocesses where this is O(1). **`uv:` WAS THE ONE EXCEPTION AND IT IS NOW CLOSED** (2026-08-29; it was called "unavoidable" here from 2026-08-23, QUEUE 44, and that was wrong): `index_unversioned()` used framed SQL to fetch NAMES only, then forked `fossil unversioned cat` per file inside its loop, because `unversioned.content` is zlib-compressed behind a 4-byte big-endian length prefix when `encoding=1` and `unversioned cat` was believed to be the only extraction path not requiring zlib. Fossil registers a `decompress()` SQL function in `add_content_sql_commands()` (`sqlcmd.c:153`) -- the same call that registers `content()` -- which strips the prefix itself, and it is available on BOTH transports (`fossil sql` registers it; so does libfossilsee, `embed/fossilsee.c:221`). viki still links no zlib. Measured with a fork-counting wrapper on a repo holding 8 uv blobs: `unversioned cat` forks 8 -> 0, total forks per index run 17 -> 9, all 8 blobs still indexed, content byte-identical to `unversioned cat`. The NUL exclusion moved into the SQL WHERE clause exactly as `attach:` already does it (the subprocess transport cannot carry an embedded NUL); such blobs were already dropped by `looks_binary()` without being marked seen, so liveness behaviour is unchanged. The old path's one virtue is gone with it and did not need replacing: it checked `unversioned cat`'s exit status and treated failure as a failure to OBSERVE rather than a withdrawal, but content now arrives with the listing, so there is no second call to fail. The KDF justification once given here was the PASSPHRASE figure -- measured 2026-08-21, a raw `x'<64 hex>'` key opens an encrypted repo in 5.99 ms vs 5.71 ms plaintext (a passphrase costs 345.93 ms), because SQLCipher skips PBKDF2 for a raw key. The framing is still right; the ~0.5 s number is not (FINDINGS.md). Also INVALIDATES: retires stale `viki_source` rows, then deletes chunks nothing references from `viki_chunk` + `chunk_fts`. Read `sweep_sources()`'s scoping comment before touching that -- it is what keeps a subdirectory index, or a machine with no `fossil`, from wiping the cache. New namespaces MUST be added to `VIRTUAL_NS[]`; `looks_like_namespace()` is the lexical safety net that stops an unknown `scheme:` key from being treated as a filesystem path and swept
...
build/dist/viki index <dir> --since auto   # INCREMENTAL: skip every extractor when nothing
                                    #   arrived above the stored blob.rcvid mark. For an
                                    #   after-receive hook, which runs SYNCHRONOUSLY in the
                                    #   pushing client's request path -- a full re-index there
                                    #   stalls the push. Measured: 0.48s on a quiet repo (one
                                    #   fossil probe) vs a full pass. NOT AUTHORITATIVE: it
                                    #   retires nothing, because it deliberately does not look
                                    #   at what did not change. The mark lives in
                                    #   .viki/rcvid.mark, a SIBLING of cache.db -- never inside
                                    #   it, because `viki cache push` ships cache.db to peers
                                    #   and rcvid is one repository's LOCAL receive order
...
something says so. Read the link back with `viki notes --closes <old-id>`,
which returns the note that retired it -- the same relation, queried instead
of grepped, so "why is this closed?" never sends you into the capture files.
...
- ~~`viki serve`'s search page isn't linked back to Fossil's own web UI~~
  **Done 2026-08-20.** Every hit now carries a `label` and a `url`, and the
  search page renders the source as an anchor. `src/viki_link.c` maps all
  nine indexed classes onto Fossil's real pages -- and the URL shapes were
  read out of the vendored fossil-see binary's own href templates
  (`strings | grep '%R/'`) rather than guessed, so they cannot drift from
  what the server actually routes. Configured by `$VIKI_FOSSIL_URL`;
  UNSET MEANS NO LINKS, and an unrecognised namespace gets no link either.
  A wrong link is worse than none: it invites citing a page that does not
  exist, with a content_hash beside it that looks like provenance.
  `build/link-probe.sh` pins one class per assertion (14/0).
...
- ~~Garbage collection of orphaned chunks~~ **Done -- and this bullet's
  ...
  in that order reversed: **`chunk_fts` FIRST**, because it is an
  external-content FTS5 table (since 2026-08-21) that re-reads
  `viki_chunk` to find the tokens it must drop. Chunk-row-first leaves
  the withdrawn text searchable while `viki_chunk` looks clean; H11
  covers it.
